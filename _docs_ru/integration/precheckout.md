...
* 3.1.На всех страницах товарного каталога, не являющихся карточками товара, вставьте следующий код:
```html
...
•	1.1. Site_id должен быть корректным, иначе будут запускать кампании, предназначенные для других сайтов;
	
•	1.2. Часто встречается ошибка, когда при вставке id ставят пробел после знака равенства. Это неправильно и код работать не будет;
	
•	1.3. Код обязательно должен находиться в header сайта.

	
*	2. Код авторизации

	
•	2.1. Для неавторизованных код срабатывать не должен;
	
•	2.2. Код должен передаваться не единожды после авторизации, а на всех страницах, где есть интеграция;
	
•	2.3. В код должны передаваться данные сессии текущего пользователя. Не должно быть передачи чужих авторизационных данных;
	
•	2.4. Должны передаваться и имя пользователя, и e-mail, если они известны;
	
•	2.5. Значения по умолчанию могут быть только пусты, т.е. равняться “”.
	
•	2.6. Если вы не знаете имя или почту пользователя, не передавайте нам их.
